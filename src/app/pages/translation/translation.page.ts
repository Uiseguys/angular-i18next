...
export class TranslationPage implements OnInit {
  ...
  ngOnInit() {
    this.route.data.subscribe(({ translation }) => {
      this.project = translation.project;
      this.translation = translation;
      // get Languages
      this.api
        .getLanguages(translation.namespace, translation.projectId)
        .subscribe(res => {
  ...
  addEntry($event) {
    ...
    this.api.addEntry(this.translation.id, this.form.value).subscribe(
      res => {
  ...
  updateEntry(key) {
    this.api
      .updateEntry(this.translation.id, {
        key,
  ...
  updateKey($event) {
    ...
    this.api.updateKey(this.translation.id, this.keyForm.value).subscribe(
      res => {
        // apply changes to local variables
  ...
  deleteEntry(key) {
    this.api.deleteEntry(this.translation.id, key).subscribe(
      res => {
  ...
  selectXLIFF() {
    if (
      !confirm(
        'The changes will be lost after importing if you have made any. Please make sure all changes are saved.'
      )
    )
      return;
    $('#file').trigger('click');
  ...
  handleXIFFSelect(event) {
    const [file] = event.target.files;
    this.api
      .importKeys(this.project.id, this.translation.namespace, file)
      .subscribe(
        res => {
          const updated = res.find(item => item.id == this.translation.id);
          this.resources = JSON.parse(updated.description || '{}');
          this.oldResources = Object.assign({}, this.resources);
          this.keys = orderBy(Object.keys(this.resources));
          this.isCollapsed = this.keys.length > 0;
        },
        err => {
          this.toasterService.pop('error', 'Sorry importing was failed');
        }
      );
  }
}
