...
export class ProjectDetailPage implements OnInit {
  ...
  editLanguage(language, namespace) {
    this.router.navigate(['./', language, namespace], {
  ...
  addNamespace($event) {
    ...
    this.api.addNamespace(namespace, this.project.id).subscribe(
      res => {
  ...
  publish(version) {
    this.api.publish(version, this.project.id).subscribe(
      res => {
        this.toasterService.pop(
          'success',
          '',
          'The projectDetail is published'
        );
      },
      res => {
        const error = JSON.parse(res._body);
  ...
  addVersion($event) {
    ...
    if (
      this.versions.find(item => item.toLowerCase() === version.toLowerCase())
    ) {
